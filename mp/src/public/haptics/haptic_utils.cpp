...
{
    // ignore idle
    if ( iActivity == ACT_VM_IDLE ) return;

#if defined( CLIENT_DLL )
    // if(hap_PrintEvents.GetBool())
    //	Msg("Client Activity :%s %s %s\n",weapon->GetName(),"Activities",VarArgs("%i",iActivity));
    if ( weapon->IsPredicted() ) haptics->ProcessHapticWeaponActivity( weapon->GetName(), iActivity );
#else
...
#ifdef CLIENT_DLL
void HapticsHandleMsg_HapSetDrag( float drag ) { haptics->SetDrag( drag ); }
#endif
...
void ConnectHaptics( CreateInterfaceFn appFactory )
{
    ...
    if ( pFalconModule )
    {
        ...
        if ( factory )
        {
            ...
            if ( haptics && haptics->Initialize( engine, view, g_InputInternal, gpGlobals, appFactory,
                                                 g_pVGuiInput->GetIMEWindow(), filesystem, enginevgui,
                                                 ActivityList_IndexForName, ActivityList_NameForIndex ) )
            {
                success = true;
...
void DisconnectHaptics()
{
    ...
    {
        // delete the stub.
        delete static_cast<CHapticsStubbed*>( haptics );
    }
    haptics = NULL;
}
...
void HapticsHandleMsg_SPHapWeapEvent( int iActivity )
{
    ...
    if ( pPlayer ) weap = pPlayer->GetActiveWeapon();
    if ( weap ) haptics->ProcessHapticEvent( 4, "Weapons", weap->GetName(), "Activities", VarArgs( "%i", iActivity ) );
}

void HapticsHandleMsg_HapPunch( QAngle const& angle ) { haptics->HapticsPunch( 1, angle ); }
#ifdef TERROR
...
void HapticsHandleMsg_HapMeleeContact() { haptics->HapticsPunch( hap_melee_scale.GetFloat(), QAngle( 0, 0, 0 ) ); }
...
void HapticsDamage( CBasePlayer* pPlayer, const CTakeDamageInfo& info )
{
    ...
    if ( !pPlayer->HasHaptics() ) return;  // do not send to non haptic users.
...
#ifdef TERROR
    else if ( ( bitDamageType & ( DMG_PARALYZE | DMG_NERVEGAS | DMG_POISON | DMG_RADIATION | DMG_DROWNRECOVER |
                                  DMG_ACID | DMG_SLOWBURN ) ) &&
              ( bitDamageType & ~( DMG_PARALYZE | DMG_NERVEGAS | DMG_POISON | DMG_RADIATION | DMG_DROWNRECOVER |
                                   DMG_ACID | DMG_SLOWBURN ) ) == 0 )
    {
...
void HapticPunch( CBasePlayer* pPlayer, float x, float y, float z ) { HapticMsg_Punch( pPlayer, x, y, z ); }

void HapticMeleeContact( CBasePlayer* pPlayer ) { HapticMsg_MeleeContact( pPlayer ); }
...
void HapticProcessSound( const char* soundname, int entIndex )
{
    ...
    if ( prediction->InPrediction() && prediction->IsFirstTimePredicted() )
    {
        ...
        if ( ent )
            local = ( entIndex == -1 || ent == C_BasePlayer::GetLocalPlayer() ||
                      ent == C_BasePlayer::GetLocalPlayer()->GetActiveWeapon() );
...
ConVar hap_ui_vehicles( "hap_ui_vehicles", HAPTIC_VEHICLE_DEFAULT, 0 );
...
void HapticsEnteredVehicle( C_BaseEntity* vehicle, C_BaseCombatCharacter* pPassenger )
{
    ...
    bool hasgun = false;
    if ( drivable ) hasgun = drivable->HasGun();
