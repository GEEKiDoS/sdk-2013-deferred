...
void DrawCube( CMeshBuilder &meshBuilder, Vector vecPos, float flRadius, float *pfl2Texcoords )
{
    const float flOffsets[24][3] = {
        -flRadius, -flRadius, -flRadius, flRadius,  -flRadius, -flRadius,
        flRadius,  flRadius,  -flRadius, -flRadius, flRadius,  -flRadius,
    ...
    for ( int i = 0; i < 24; i++ )
    {
        meshBuilder.Position3f( vecPos.x + flOffsets[i][0], vecPos.y + flOffsets[i][1], vecPos.z + flOffsets[i][2] );
...
public:
    ...
    void Setup( const CViewSetup &view, int nClearFlags, bool bDrawSkybox, const VisibleFogVolumeInfo_t &fogInfo,
                const WaterRenderInfo_t &info, ViewCustomVisibility_t *pCustomVisibility = NULL );
    void Draw();
...
public:
    CSkyboxViewDeferred( CViewRender *pMainView ) : CGBufferView( pMainView ), m_pSky3dParams( NULL ) {}
...
protected:
    ...
    void DrawInternal( view_id_t iSkyBoxViewID = VIEW_3DSKY, ITexture *pRenderTarget = NULL,
...
public:
    CBaseShadowView( CViewRender *pMainView ) : CBaseWorldViewDeferred( pMainView ) { m_bOutputRadiosity = false; };

    void Setup( const CViewSetup &view, ITexture *pDepthTexture, ITexture *pDummyTexture );
    void SetupRadiosityTargets( ITexture *pAlbedoTexture, ITexture *pNormalTexture );
...
public:
    COrthoShadowView( CViewRender *pMainView, const int &index ) : CBaseShadowView( pMainView )
    {
        iCascadeIndex = index;
    }
...
public:
    CDualParaboloidShadowView( CViewRender *pMainView, def_light_t *pLight, const bool &bSecondary )
    ...
    virtual int GetShadowMode() { return DEFERRED_SHADOW_MODE_DPSM; };
...
public:
    CSpotLightShadowView( CViewRender *pMainView, def_light_t *pLight, int index ) : CBaseShadowView( pMainView )
    {
        m_pLight = pLight;
        m_iIndex = index;
    }
    ...
    virtual int GetShadowMode() { return DEFERRED_SHADOW_MODE_PROJECTED; };
...
public:
    CBaseWaterViewDeferred( CViewRender *pMainView )
        : CBaseWorldViewDeferred( pMainView ), m_SoftwareIntersectionView( pMainView )
    {
    }
...
protected:
    void CalcWaterEyeAdjustments( const VisibleFogVolumeInfo_t &fogInfo, float &newWaterHeight, float &waterZAdjust,
...
public:
    ...
    void Setup( const CViewSetup &view, bool bDrawSkybox, const VisibleFogVolumeInfo_t &fogInfo,
...
public:
    CUnderWaterViewDeferred( CViewRender *pMainView )
        : CBaseWaterViewDeferred( pMainView ), m_RefractionView( pMainView )
    {
    }

    void Setup( const CViewSetup &view, bool bDrawSkybox, const VisibleFogVolumeInfo_t &fogInfo,
                const WaterRenderInfo_t &info );
    void Draw();
                                        ...
                                        bool bShadowDepth )
{
    ...
    BuildWorldRenderLists( bDrawEntities, iForceViewLeaf, ShouldCacheLists(), false,
                           bDrawReflection ? &waterHeight : NULL );
...
void CBaseWorldViewDeferred::DrawExecute( float waterHeight, view_id_t viewID, float waterZAdjust, bool bShadowDepth )
{
    ...
    // if (!m_bDrawWorldNormal)
    {
        if ( m_DrawFlags & DF_DRAW_ENTITITES )
        {
            ...
            if ( !bShadowDepth ) DrawNoZBufferTranslucentRenderables();
        }
        else
        {
...
void CBaseWorldViewDeferred::DrawWorldDeferred( float waterZAdjust ) { DrawWorld( waterZAdjust ); }
...
{
    CMatRenderContextPtr pRenderContext( materials );
    pRenderContext->SetIntRenderingParameter( INT_RENDERPARM_DEFERRED_RENDER_STAGE, DEFERRED_RENDER_STAGE_COMPOSITION );
}

void CBaseWorldViewDeferred::PopComposite()
{
    CMatRenderContextPtr pRenderContext( materials );
    pRenderContext->SetIntRenderingParameter( INT_RENDERPARM_DEFERRED_RENDER_STAGE, DEFERRED_RENDER_STAGE_INVALID );
}
                                      ...
                                      ViewCustomVisibility_t *pCustomVisibility )
{
    ...
    {
        const bool bViewIntersectsWater =
...
void CGBufferView::Setup( const CViewSetup &view, bool bDrewSkybox )
{
    ...
    if ( !bDrewSkybox ) m_DrawFlags |= DF_DRAWSKYBOX;
#endif
}
...
void CGBufferView::PushView( float waterHeight ) { PushGBuffer( !m_bDrewSkybox ); }

void CGBufferView::PopView() { PopGBuffer(); }
...
void CGBufferView::PushGBuffer( bool bInitial, float zScale, bool bClearDepth )
{
    ...
    public:
        ...
        static void Fire( defData_setZScale d ) { GetDeferredExt()->CommitZScale( d.zScale ); };
    };
...
{
    if ( ( enginetrace->GetPointContents( origin ) & CONTENTS_SOLID ) != 0 ) return SKYBOX_NOT_VISIBLE;
...
void CSkyboxViewDeferred::DrawInternal( view_id_t iSkyBoxViewID, ITexture *pRenderTarget, ITexture *pDepthTarget )
{
    ...
    *areabits = tmpbits;
    ...
    if ( !m_bGBufferPass ) Enable3dSkyboxFog();
...
void CPostLightingView::PopView() { BaseClass::PopView(); }
...
void CBaseShadowView::Draw()
{
    ...
    CMatRenderContextPtr pRenderContext( materials );
    pRenderContext->SetIntRenderingParameter( INT_RENDERPARM_DEFERRED_RENDER_STAGE, DEFERRED_RENDER_STAGE_SHADOWPASS );
    pRenderContext->SetIntRenderingParameter( INT_RENDERPARM_DEFERRED_SHADOW_MODE, GetShadowMode() );
    pRenderContext->SetIntRenderingParameter( INT_RENDERPARM_DEFERRED_SHADOW_RADIOSITY, m_bOutputRadiosity ? 1 : 0 );
    pRenderContext.SafeRelease();
    ...
    pRenderContext.GetFrom( materials );
    pRenderContext->SetIntRenderingParameter( INT_RENDERPARM_DEFERRED_SHADOW_RADIOSITY, 0 );
    pRenderContext->SetIntRenderingParameter( INT_RENDERPARM_DEFERRED_RENDER_STAGE, DEFERRED_RENDER_STAGE_INVALID );
...
void CBaseShadowView::SetRadiosityOutputEnabled( bool bEnabled ) { m_bOutputRadiosity = bEnabled; }
...
void COrthoShadowView::CalcShadowView()
{
    ...
    origin +=
        -viewFwd * m_data.flOriginOffset + viewUp * halfOrthoSize - viewRight * halfOrthoSize + mainFwd * halfOrthoSize;
...
void COrthoShadowView::CommitData()
{
    struct sendShadowDataOrtho
    {
        ...
        int index;
        static void Fire( sendShadowDataOrtho d ) { GetDeferredExt()->CommitShadowData_Ortho( d.index, d.data ); };
    };
    ...
    shadowData.vecSlopeSettings.Init( data.flSlopeScaleMin, data.flSlopeScaleMax, data.flNormalScaleMax, 1.0f / zFar );
    shadowData.vecOrigin.Init( origin );

    Vector4D matrix_scale_offset( 0.5f, -0.5f, 0.5f, 0.5f );

#if CSM_USE_COMPOSITED_TARGET
    shadowData.vecUVTransform.Init( x / (float) CSM_COMP_RES_X, y / (float) CSM_COMP_RES_Y,
                                    width / (float) CSM_COMP_RES_X, height / (float) CSM_COMP_RES_Y );
#endif

    VMatrix a, b, c, d, screenToTexture;
    render->GetMatricesForView( *this, &a, &b, &c, &d );
    MatrixBuildScale( screenToTexture, matrix_scale_offset.x, matrix_scale_offset.y, 1.0f );
...
void CDualParaboloidShadowView::PushView( float waterHeight ) { BaseClass::PushView( waterHeight ); }

void CDualParaboloidShadowView::PopView() { BaseClass::PopView(); }
...
void CSpotLightShadowView::CommitData()
{
    struct sendShadowDataProj
    {
        ...
        int index;
        static void Fire( sendShadowDataProj d ) { GetDeferredExt()->CommitShadowData_Proj( d.index, d.data ); };
    };
...
void CDeferredViewRender::Shutdown()
{
    ...
    {
        if ( mesh != NULL ) pRenderContext->DestroyStaticMesh( mesh );
...
void CDeferredViewRender::LevelInit() { BaseClass::LevelInit(); }

void CDeferredViewRender::ViewDrawSceneDeferred( const CViewSetup &view, int nClearFlags, view_id_t viewID,
                                                 bool bDrawViewModel )
{
    ...
    if ( deferred_radiosity_debug.GetBool() ) DebugRadiosity( view );
#endif
...
void CDeferredViewRender::ViewDrawGBuffer( const CViewSetup &view, bool &bDrew3dSkybox,
                                           SkyboxVisibility_t &nSkyboxVisible, bool bDrawViewModel )
{
    ...
    if ( ( bDrew3dSkybox = pSkyView->Setup( view, true, &nSkyboxVisible ) ) != false ) AddViewToScene( pSkyView );
...
void CDeferredViewRender::ViewDrawComposite( const CViewSetup &view, bool &bDrew3dSkybox,
                                             SkyboxVisibility_t &nSkyboxVisible, int nClearFlags, view_id_t viewID,
                                             bool bDrawViewModel )
{
    ...
    if ( !bDrew3dSkybox && ( nSkyboxVisible == SKYBOX_NOT_VISIBLE ) &&
         ( visFlags & IVRenderView::VIEW_SETUP_VIS_EX_RETURN_FLAGS_USES_RADIAL_VIS ) )
    {
    ...
    if ( bDrew3dSkybox || ( nSkyboxVisible == SKYBOX_NOT_VISIBLE ) ) drawSkybox = false;
...
void CDeferredViewRender::ViewCombineDeferredShading( const CViewSetup &view, view_id_t viewID )
{
    ...
    DrawLightPassFullscreen( GetDeferredManager()->GetDeferredMaterial( DEF_MAT_SCREENSPACE_SHADING ), view.width,
                             view.height );
...
{
#if DEFCFG_DEFERRED_SHADING
    DrawLightPassFullscreen( GetDeferredManager()->GetDeferredMaterial( DEF_MAT_SCREENSPACE_COMBINE ), view.width,
                             view.height );
#endif
}
...
void CDeferredViewRender::PerformLighting( const CViewSetup &view )
{
    ...
    if ( bRadiosityEnabled ) BeginRadiosity( view );
    ...
    if ( GetGlobalLight() != NULL )
    {
        ...
        if ( !GetLightingEditor()->IsEditorLightingActive() && deferred_override_globalLight_enable.GetBool() )
        {
            ...
            UTIL_StringToVector( lightDataState.diff.AsVector3D().Base(),
                                 deferred_override_globalLight_diffuse.GetString() );
            UTIL_StringToVector( lightDataState.ambh.AsVector3D().Base(),
                                 deferred_override_globalLight_ambient_high.GetString() );
            UTIL_StringToVector( lightDataState.ambl.AsVector3D().Base(),
                                 deferred_override_globalLight_ambient_low.GetString() );

            lightDataState.bEnabled =
                ( lightDataState.diff.LengthSqr() > 0.01f || lightDataState.ambh.LengthSqr() > 0.01f ||
    ...
    if ( building_cubemaps.GetBool() ) engine->GetScreenSize( lightingView.width, lightingView.height );
    ...
    if ( bResetLightAccum )
    {
        ...
        DrawLightPassFullscreen( GetDeferredManager()->GetDeferredMaterial( DEF_MAT_LIGHT_GLOBAL ), lightingView.width,
                                 lightingView.height );
    ...
    if ( bRadiosityEnabled ) EndRadiosity( view );
...
static int GetSourceRadBufferIndex( const int index )
{
    ...
    const int iNumSteps =
        ( bFar ? deferred_radiosity_propagate_count_far.GetInt() : deferred_radiosity_propagate_count.GetInt() ) +
...
void CDeferredViewRender::BeginRadiosity( const CViewSetup &view )
{
    ...
    for ( int iCascade = 0; iCascade < 2; iCascade++ )
    {
        ...
        const Vector gridSize( RADIOSITY_BUFFER_SAMPLES_XY, RADIOSITY_BUFFER_SAMPLES_XY, RADIOSITY_BUFFER_SAMPLES_Z );
        const Vector gridSizeHalf = gridSize / 2;
        const float gridStepSize = bFar ? RADIOSITY_BUFFER_GRID_STEP_SIZE_FAR : RADIOSITY_BUFFER_GRID_STEP_SIZE_CLOSE;
        const float flGridDistance =
            bFar ? RADIOSITY_BUFFER_GRID_STEP_DISTANCEMULT_FAR : RADIOSITY_BUFFER_GRID_STEP_DISTANCEMULT_CLOSE;

        Vector vecFwd;
        AngleVectors( view.angles, &vecFwd );

        m_vecRadiosityOrigin[iCascade] =
            view.origin + vecFwd * gridStepSize * RADIOSITY_BUFFER_SAMPLES_XY * flGridDistance * flAmtVertical;
        ...
        pRenderContext->PushRenderTargetAndViewport( GetDefRT_RadiosityBuffer( iSourceBuffer ), NULL, 0, clearOffset,
                                                     RADIOSITY_BUFFER_RES_X, clearSizeY );
        ...
        if ( iLastSourceBuffer[iCascade] != iSourceBuffer )
        {
            ...
            pRenderContext->PushRenderTargetAndViewport( GetDefRT_RadiosityBuffer( 1 - iSourceBuffer ), NULL, 0,
                                                         clearOffset, RADIOSITY_BUFFER_RES_X, clearSizeY );
            pRenderContext->ClearColor3ub( 0, 0, 0 );
            pRenderContext->ClearBuffers( true, false );
            pRenderContext->PopRenderTargetAndViewport();

            pRenderContext->PushRenderTargetAndViewport( GetDefRT_RadiosityNormal( 1 - iSourceBuffer ), NULL, 0,
                                                         clearOffset, RADIOSITY_BUFFER_RES_X, clearSizeY );
        ...
        pRenderContext->PushRenderTargetAndViewport( GetDefRT_RadiosityNormal( iSourceBuffer ), NULL, 0, clearOffset,
                                                     RADIOSITY_BUFFER_RES_X, clearSizeY );
        pRenderContext->ClearColor3ub( 127, 127, 127 );
        pRenderContext->ClearBuffers( true, false );
        pRenderContext->PopRenderTargetAndViewport();
...
void CDeferredViewRender::PerformRadiosityGlobal( const int iRadiosityCascade, const CViewSetup &view )
{
    ...
    pRenderContext->PushRenderTargetAndViewport( GetDefRT_RadiosityBuffer( iSourceBuffer ), NULL, 0, iOffsetY,
                                                 RADIOSITY_BUFFER_VIEWPORT_SX, RADIOSITY_BUFFER_VIEWPORT_SY );
...
void CDeferredViewRender::EndRadiosity( const CViewSetup &view )
{
    ...
    for ( int iCascade = 0; iCascade < 2; iCascade++ )
    {
        ...
        for ( int i = 0; i < iNumPropagateSteps[iCascade]; i++ )
        {
            ...
            CMatRenderContextPtr pRenderContext( materials );
            pRenderContext->PushRenderTargetAndViewport( GetDefRT_RadiosityBuffer( index ), NULL, 0, iOffsetY,
                                                         RADIOSITY_BUFFER_VIEWPORT_SX, RADIOSITY_BUFFER_VIEWPORT_SY );
    ...
    if ( m_hRadiosityDebugMeshList[0].Count() == 0 )
    {
        for ( int iCascade = 0; iCascade < 2; iCascade++ )
        {
            ...
            IMesh *pMesh = pRenderContext->CreateStaticMesh( VERTEX_POSITION | VERTEX_TEXCOORD_SIZE( 0, 2 ),
                                                             TEXTURE_GROUP_OTHER, pMatDbgRadGrid );
            ...
            if ( nMaxCubes > nMaxVerts / 24 ) nMaxCubes = nMaxVerts / 24;

            int nRenderRemaining = nMaxCubes;
            meshBuilder.Begin( pMesh, MATERIAL_QUADS, nMaxCubes * 6 );

            const Vector2D flUVTexelSize( 1.0f / RADIOSITY_BUFFER_RES_X, 1.0f / RADIOSITY_BUFFER_RES_Y );
            const Vector2D flUVTexelSizeHalf = flUVTexelSize * 0.5f;
            const Vector2D flUVGridSize =
                Vector2D( RADIOSITY_UVRATIO_X, RADIOSITY_UVRATIO_Y ) * 1.0f / RADIOSITY_BUFFER_GRIDS_PER_AXIS;
                ...
                for ( int y = 0; y < RADIOSITY_BUFFER_SAMPLES_XY; y++ )
                    for ( int z = 0; z < RADIOSITY_BUFFER_SAMPLES_Z; z++ )
                    {
                        if ( nRenderRemaining <= 0 )
                        {
                            ...
                            meshBuilder.End();
                            pMesh = pRenderContext->CreateStaticMesh(
                                VERTEX_POSITION | VERTEX_TEXCOORD_SIZE( 0, 2 ), TEXTURE_GROUP_OTHER,
                        ...
                        DrawCube( meshBuilder, directions[0] * x + directions[1] * y + directions[2] * z, flCubesize,
                                  flUV );

                        nRenderRemaining--;
                    }

            if ( nRenderRemaining != nMaxCubes ) meshBuilder.End();
        }
    }
...
void CDeferredViewRender::RenderCascadedShadows( const CViewSetup &view, const bool bEnableRadiosity )
{
    for ( int i = 0; i < SHADOW_NUM_CASCADES; i++ )
    {
        ...
        if ( bDoRadiosity ) PerformRadiosityGlobal( iRadTarget, view );
    }
}
...
void CDeferredViewRender::DrawLightShadowView( const CViewSetup &view, int iDesiredShadowmap, def_light_t *l )
{
    ...
    switch ( l->iLighttype )
    {
        ...
        case DEFLIGHTTYPE_POINT: {
            ...
            pDPView0->Setup( setup, GetShadowDepthRT_DP( iDesiredShadowmap ),
                             GetShadowColorRT_DP( iDesiredShadowmap ) );
            AddViewToScene( pDPView0 );

            CRefPtr<CDualParaboloidShadowView> pDPView1 = new CDualParaboloidShadowView( this, l, true );
            pDPView1->Setup( setup, GetShadowDepthRT_DP( iDesiredShadowmap ),
                             GetShadowColorRT_DP( iDesiredShadowmap ) );
        ...
        case DEFLIGHTTYPE_SPOT: {
            ...
            pProjView->Setup( setup, GetShadowDepthRT_Proj( iDesiredShadowmap ),
                              GetShadowColorRT_Proj( iDesiredShadowmap ) );
...
void CDeferredViewRender::DrawViewModels( const CViewSetup &view, bool drawViewmodel, bool bGBuffer )
{
    ...
    {
        pRTColor = g_pSourceVR->GetRenderTarget( ( ISourceVirtualReality::VREye )( view.m_eStereoEye - 1 ),
                                                 ISourceVirtualReality::RT_Color );
        pRTDepth = g_pSourceVR->GetRenderTarget( ( ISourceVirtualReality::VREye )( view.m_eStereoEye - 1 ),
                                                 ISourceVirtualReality::RT_Depth );
    }
...
#ifdef PORTAL  // the depth range hack doesn't work well enough for the portal mod (and messing with the depth hack
               // values makes some models draw incorrectly) step up to a full depth clear if we're extremely close to a
               // portal (in a portal environment)
    extern bool LocalPlayerIsCloseToPortal(
        void );  // defined in C_Portal_Player.cpp, abstracting to a single bool function to remove explicit dependence
                 // on c_portal_player.h/cpp, you can define the function as a "return true" in other build
                 // configurations at the cost of some perf
    bool bUseDepthHack = !LocalPlayerIsCloseToPortal();
    if ( !bUseDepthHack ) pRenderContext->ClearBuffers( false, true, false );
#else
...
void CDeferredViewRender::RenderView( const CViewSetup &view, int nClearFlags, int whatToDraw )
{
    ...
    if ( !building_cubemaps.GetBool() &&
         worldView.m_bDoBloomAndToneMapping )  // We probably should use a different view. variable here
    {
    ...
    if ( mat_viewportupscale.GetBool() && mat_viewportscale.GetFloat() < 1.0f )
    {
        ...
        pRenderContext->DrawScreenSpaceRectangle(
            pCopyMaterial, UpscaleRect.x, UpscaleRect.y, UpscaleRect.width, UpscaleRect.height, DownscaleRect.x,
            DownscaleRect.y, DownscaleRect.x + DownscaleRect.width - 1, DownscaleRect.y + DownscaleRect.height - 1,
    ...
    if ( UseVR() )
    {
        saveRenderTarget = g_pSourceVR->GetRenderTarget( ( ISourceVirtualReality::VREye )( worldView.m_eStereoEye - 1 ),
                                                         ISourceVirtualReality::RT_Color );
    }
    ...
    if ( whatToDraw & RENDERVIEW_DRAWHUD )
    {
        ...
        if ( bPaintMainMenu ) render->VGui_Paint( PAINT_UIPANELS | PAINT_CURSOR );
        ...
        if ( UseVR() )
        {
            ...
            if ( !bInFreezeCam && g_ClientVirtualReality.ShouldRenderHUDInWorld() )
            {
                ...
                // TODO - a bit of a shonky test - basically trying to catch the main menu, the briefing screen, the
                // loadout screen, etc.
...
void CDeferredViewRender::ProcessDeferredGlobals( const CViewSetup &view )
{
    ...
    MatrixBuildPerspectiveX( matPerspective, view.fov, view.m_flAspectRatio, view.zNear, view.zFar );
    ...
    QUEUE_FIRE( CommitCommonData, view.origin, fwd, view.zNear, view.zFar, frustumDeltas.Transpose3x3(),
                matWorldToCameraDepthTex.Transpose() );
#else
    QUEUE_FIRE( CommitCommonData, view.origin, fwd, view.zNear, view.zFar, frustumDeltas.Transpose3x3() );
#endif
}
...
IMesh *CDeferredViewRender::CreateRadiosityScreenGrid( const Vector2D &vecViewportBase, const float flWorldStepSize )
{
    VertexFormat_t format =
        VERTEX_POSITION | VERTEX_TEXCOORD_SIZE( 0, 4 ) | VERTEX_TEXCOORD_SIZE( 1, 4 ) | VERTEX_TANGENT_S;

    const float flTexelGridMargin = 1.5f / RADIOSITY_BUFFER_SAMPLES_XY;
    const float flTexelHalf[2] = { 0.5f / RADIOSITY_BUFFER_VIEWPORT_SX, 0.5f / RADIOSITY_BUFFER_VIEWPORT_SY };

    const float flLocalCoordSingle = 1.0f / RADIOSITY_BUFFER_GRIDS_PER_AXIS;
    const float flLocalCoords[4][2] = {
        0, 0, flLocalCoordSingle, 0, flLocalCoordSingle, flLocalCoordSingle, 0, flLocalCoordSingle,
    };

    CMatRenderContextPtr pRenderContext( materials );
    IMesh *pRet = pRenderContext->CreateStaticMesh( format, TEXTURE_GROUP_OTHER );

    CMeshBuilder meshBuilder;
    meshBuilder.Begin( pRet, MATERIAL_QUADS, RADIOSITY_BUFFER_GRIDS_PER_AXIS * RADIOSITY_BUFFER_GRIDS_PER_AXIS );
    ...
    const float flLocalGridSize[4][2] = {
        0, 0, flGridSize, 0, flGridSize, flGridSize, 0, flGridSize,
    };
    const float flLocalGridLimits[4][2] = {
        -flTexelGridMargin,    -flTexelGridMargin,    1 + flTexelGridMargin, -flTexelGridMargin,
        1 + flTexelGridMargin, 1 + flTexelGridMargin, -flTexelGridMargin,    1 + flTexelGridMargin,
    };
    ...
    for ( int x = 0; x < RADIOSITY_BUFFER_GRIDS_PER_AXIS; x++ )
    {
        for ( int y = 0; y < RADIOSITY_BUFFER_GRIDS_PER_AXIS; y++ )
        {
            ...
            const int iIndicesOne[2] = { Min( RADIOSITY_BUFFER_SAMPLES_Z - 1, iIndexLocal + 1 ),
                                         Max( 0, iIndexLocal - 1 ) };

            for ( int q = 0; q < 4; q++ )
            {
                meshBuilder.Position3f( ( x * flLocalCoordSingle + flLocalCoords[q][0] ) * 2 -
                                            flLocalCoordSingle * RADIOSITY_BUFFER_GRIDS_PER_AXIS,
                                        flLocalCoordSingle * RADIOSITY_BUFFER_GRIDS_PER_AXIS -
                                            ( y * flLocalCoordSingle + flLocalCoords[q][1] ) * 2,
                                        0 );
                ...
                meshBuilder.TexCoord4f(
                    1,
                    ( flGridOrigins[iIndicesOne[0]][0] + flLocalCoords[q][0] ) * RADIOSITY_UVRATIO_X +
                        vecViewportBase.x,
                    ( flGridOrigins[iIndicesOne[0]][1] + flLocalCoords[q][1] ) * RADIOSITY_UVRATIO_Y +
                        vecViewportBase.y,
                    ( flGridOrigins[iIndicesOne[1]][0] + flLocalCoords[q][0] ) * RADIOSITY_UVRATIO_X +
                        vecViewportBase.x,
                    ( flGridOrigins[iIndicesOne[1]][1] + flLocalCoords[q][1] ) * RADIOSITY_UVRATIO_Y +
                        vecViewportBase.y );

                meshBuilder.TangentS3f( flLocalGridSize[q][0], flLocalGridSize[q][1], iIndexLocal * flWorldStepSize );
...
//
//-----------------------------------------------------------------------------
void CBaseWaterViewDeferred::CalcWaterEyeAdjustments( const VisibleFogVolumeInfo_t &fogInfo, float &newWaterHeight,
                                                      float &waterZAdjust, bool bSoftwareUserClipPlane )
{
...
//-----------------------------------------------------------------------------
void CAboveWaterViewDeferred::Setup( const CViewSetup &view, bool bDrawSkybox, const VisibleFogVolumeInfo_t &fogInfo,
                                     const WaterRenderInfo_t &waterInfo )
{
    BaseClass::Setup( view );
...
#ifdef PORTAL
    if ( g_pPortalRender->ShouldObeyStencilForClears() ) m_ClearFlags |= VIEW_CLEAR_OBEY_STENCIL;
#endif
...
void CAboveWaterViewDeferred::Draw()
{
    ...
    if ( m_waterInfo.m_bRefract )
    {
        ...
        {
            bViewIntersectsWater =
                DoesViewPlaneIntersectWater( m_fogInfo.m_flWaterHeight, m_fogInfo.m_nVisibleFogVolume );
        }
    }
...
void CAboveWaterViewDeferred::CReflectionView::Draw()
{
    ...
    DrawSetup( GetOuter()->m_fogInfo.m_flWaterHeight, m_DrawFlags, 0.0f,
               GetOuter()->m_fogInfo.m_nVisibleFogVolumeLeaf );
...
void CAboveWaterViewDeferred::CRefractionView::Setup()
{
    ...
    m_DrawFlags = DF_RENDER_REFRACTION | DF_CLIP_Z | DF_RENDER_UNDERWATER | DF_FUDGE_UP | DF_DRAW_ENTITITES;
}
...
//-----------------------------------------------------------------------------
void CUnderWaterViewDeferred::Setup( const CViewSetup &view, bool bDrawSkybox, const VisibleFogVolumeInfo_t &fogInfo,
                                     const WaterRenderInfo_t &waterInfo )
{
    BaseClass::Setup( view );
...
void CUnderWaterViewDeferred::CRefractionView::Setup()
{
    ...
    m_DrawFlags = DF_CLIP_Z | DF_CLIP_BELOW | DF_RENDER_ABOVEWATER | DF_DRAW_ENTITITES;
