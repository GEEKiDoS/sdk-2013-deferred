...
{
    lightDataCommon_t( size_t dataCount ) { pFlData = new float[dataCount]; }
    void DeleteThis()
    {
        delete[] pFlData;
        delete this;
...
public:
    ...
    virtual void CommitTexture_Radiosity( ITexture *pTexRadBuffer0, ITexture *pTexRadBuffer1, ITexture *pTexRadNormal0,
                                          ITexture *pTexRadNormal1 ) = 0;
};
...
public:
    ...
    void CommitCommonData( const Vector &origin, const Vector &fwd, const float &zNear, const float &zFar,
                           const VMatrix &matTFrustum
#if DEFCFG_BILATERAL_DEPTH_TEST
                           ,
                           const VMatrix &matWorldCameraDepthTex
...
float *CDeferredExtension::GetOriginBase() { return m_vecOrigin.Base(); }
float *CDeferredExtension::GetForwardBase() { return m_vecForward.Base(); }
const float &CDeferredExtension::GetZDistNear() { return m_flZDists[0]; }
const float &CDeferredExtension::GetZDistFar() { return m_flZDists[1]; }
float CDeferredExtension::GetZScale() { return m_flZDists[2]; }
float *CDeferredExtension::GetFrustumDeltaBase() { return m_matTFrustumD.Base(); }

#if DEFCFG_BILATERAL_DEPTH_TEST
float *CDeferredExtension::GetWorldToCameraDepthTexBase() { return m_matWorldCameraDepthTex.Base(); }
#endif
...
const shadowData_general_t &CDeferredExtension::GetShadowData_General() { return m_dataGeneral; }

const lightData_Global_t &CDeferredExtension::GetLightData_Global() { return m_globalLight; }

const volumeData_t &CDeferredExtension::GetVolumeData() { return m_dataVolume; }

const radiosityData_t &CDeferredExtension::GetRadiosityData() { return m_dataRadiosity; }

int CDeferredExtension::GetNumActiveLights_ShadowedCookied() { return m_iNumCommon_ShadowedCookied; }
int CDeferredExtension::GetNumActiveLights_Shadowed() { return m_iNumCommon_Shadowed; }
int CDeferredExtension::GetNumActiveLights_Cookied() { return m_iNumCommon_Cookied; }
int CDeferredExtension::GetNumActiveLights_Simple() { return m_iNumCommon_Simple; }
float *CDeferredExtension::GetActiveLightData()
{
    if ( m_pflCommonLightData ) return m_pflCommonLightData->pFlData;
    return NULL;
}
int CDeferredExtension::GetActiveLights_NumRows() { return m_iCommon_NumRows; }
...
#elif DEFCFG_DEFERRED_SHADING == 1
ITexture *CDeferredExtension::GetTexture_Albedo() { return m_pTexAlbedo; }
ITexture *CDeferredExtension::GetTexture_Specular() { return m_pTexSpecular; }
#endif
...
ITexture *CDeferredExtension::GetTexture_VolumePrePass() { return m_pTexVolumePrePass; }
ITexture *CDeferredExtension::GetTexture_ShadowRad_Ortho_Albedo() { return m_pTexShadowRad_Ortho[0]; }
ITexture *CDeferredExtension::GetTexture_ShadowRad_Ortho_Normal() { return m_pTexShadowRad_Ortho[1]; }
...
FORCEINLINE CDeferredExtension *GetDeferredExt() { return &__g_defExt; }
#endif
