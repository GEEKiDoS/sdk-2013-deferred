...
void CDeferredExtension::EnableDeferredLighting() { m_bDefLightingEnabled = true; }

bool CDeferredExtension::IsDeferredLightingEnabled() const { return m_bDefLightingEnabled; }
...
void CDeferredExtension::CommitCommonData( const Vector &origin, const Vector &fwd, const float &zNear,
                                           const float &zFar, const VMatrix &matTFrustum
...
void CDeferredExtension::CommitZScale( const float &zFar ) { m_flZDists[2] = zFar; }
...
void CDeferredExtension::CommitShadowData_General( const shadowData_general_t &data ) { m_dataGeneral = data; }

void CDeferredExtension::CommitVolumeData( const volumeData_t &data ) { m_dataVolume = data; }

void CDeferredExtension::CommitRadiosityData( const radiosityData_t &data ) { m_dataRadiosity = data; }

void CDeferredExtension::CommitLightData_Global( const lightData_Global_t &data ) { m_globalLight = data; }

void CDeferredExtension::CommitLightData_Common( lightDataCommon_t *pData )
{
    if ( m_pflCommonLightData ) m_pflCommonLightData->DeleteThis();
...
void CDeferredExtension::CommitTexture_SpecularRoughness( ITexture *m_pTexSpecularRough ) { m_pTexSpecularRoughness = m_pTexSpecularRough; }
