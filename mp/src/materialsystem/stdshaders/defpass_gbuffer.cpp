...
{
    if ( PARM_DEFINED( info.iBumpmap ) ) pShader->LoadBumpMap( info.iBumpmap );

    if ( PARM_DEFINED( info.iBumpmap2 ) ) pShader->LoadBumpMap( info.iBumpmap2 );
                      ...
                      VertexCompressionType_t vertexCompression, CDeferredPerMaterialContextData *pDeferredContext )
{
    ...
    SHADOW_STATE
    {
        ...
        int *pTexCoordDim;
        int iTexCoordNum;
        GetTexcoordSettings( bModel, ( bModel && bIsDecal && bFastVTex ), iTexCoordNum, &pTexCoordDim );
        ...
        {
            if ( bBumpmap2 || bAlbedo2 ) iVFmtFlags |= VERTEX_COLOR;
        }
    ...
    DYNAMIC_STATE
    {
        ...
        if ( pDeferredContext->m_bMaterialVarsChanged ||
             !pDeferredContext->HasCommands( CDeferredPerMaterialContextData::DEFSTAGE_GBUFFER ) )
        {
            tmpBuf.Reset();
        ...
        if ( bModel && bFastVTex )
            pShader->SetHWMorphVertexShaderState( VERTEX_SHADER_SHADER_SPECIFIC_CONST_10,
                                                  VERTEX_SHADER_SHADER_SPECIFIC_CONST_11,
...
{
    return ( litface + half_lambert * 2 + phong_exp * 4 ) / 255.0f;
}

void UnpackLightingControls( float mixed, float &phong_exp, float &half_lambert, float &litface )
{
    mixed *= 255.0f;
...
CON_COMMAND( test_packing, "" )
{
    ...
    float res = PackLightingControls( atoi( args[1] ), atoi( args[2] ), atoi( args[3] ) );
