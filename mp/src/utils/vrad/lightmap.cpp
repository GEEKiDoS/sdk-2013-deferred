...
fltx4 CalculateAmbientOcclusion4( const FourVectors &position4, const FourVectors &normal4,
...
int CNormalList::FindOrAddNormal( Vector const &vNormal )
{
    ...
    for ( int i = 0; i < pGridElement->Size(); i++ )
    {
        ...
        if ( *pVec == vNormal ) return iNormal;
    }
...
void GetBumpNormals( const float *sVect, const float *tVect, const Vector &flatNormal, const Vector &phongNormal,
    ...
    for ( i = 0, f = g_pFaces; i < numfaces; i++, f++ )
    {
        ...
        for ( j = 0; j < f->numedges; j++ )
        {
            ...
            for ( k = 0; k < vertexref[n]; k++ )
            {
                ...
                // skip self
                if ( vertexface[n][k] == i ) continue;

                // if this face doens't have a displacement -- don't consider displacement neighbors
                if ( ( !fn->bHasDisp ) && ( faceneighbor[vertexface[n][k]].bHasDisp ) ) continue;
                ...
                else
                {
                    ...
                    else
                    {
                        unsigned int smoothingGroup =
                            ( f->smoothingGroups & g_pFaces[vertexface[n][k]].smoothingGroups );

                        // Hard edge.
                        if ( ( smoothingGroup & SMOOTHING_GROUP_HARD_EDGE ) != 0 ) continue;
                ...
                {
                    if ( tmpneighbor[m] == vertexface[n][k] ) break;
                }
...
void ErrorLightInfo( const char *s, lightinfo_t *l )
{
    ...
    if ( w != NULL )
    {
        ...
        Warning( "%s at (%g, %g, %g)\n\tmaterial=%s\n", s, (double) vecCenter.x, (double) vecCenter.y,
                 (double) vecCenter.z, TexDataStringTable_GetString( dtexdata[tex->texdata].nameStringTableID ) );
    }
    //
...
void CalcFaceVectors( lightinfo_t *l )
{
    ...
    // Solve[ { x * w00 + y * w01 + z * w02 - s == 0, x * w10 + y * w11 + z * w12 - t == 0, A * x + B * y + C * z + D ==
    // 0 }, { x, y, z } ] Rule(x,(  C*s*w11 - B*s*w12 + B*t*w02 - C*t*w01 + D*w02*w11 - D*w01*w12) / (+ A*w01*w12 -
    // A*w02*w11 + B*w02*w10 - B*w00*w12 + C*w00*w11 - C*w01*w10 )), Rule(y,(  A*s*w12 - C*s*w10 + C*t*w00 - A*t*w02 +
    // D*w00*w12 - D*w02*w10) / (+ A*w01*w12 - A*w02*w11 + B*w02*w10 - B*w00*w12 + C*w00*w11 - C*w01*w10 )), Rule(z,(
    // B*s*w10 - A*s*w11 + A*t*w01 - B*t*w00 + D*w01*w10 - D*w00*w11) / (+ A*w01*w12 - A*w02*w11 + B*w02*w10 - B*w00*w12
    // + C*w00*w11 - C*w01*w10 ))))

    Vector luxelSpaceCross;

    luxelSpaceCross[0] = tex->lightmapVecsLuxelsPerWorldUnits[1][1] * tex->lightmapVecsLuxelsPerWorldUnits[0][2] -
                         tex->lightmapVecsLuxelsPerWorldUnits[1][2] * tex->lightmapVecsLuxelsPerWorldUnits[0][1];
    luxelSpaceCross[1] = tex->lightmapVecsLuxelsPerWorldUnits[1][2] * tex->lightmapVecsLuxelsPerWorldUnits[0][0] -
                         tex->lightmapVecsLuxelsPerWorldUnits[1][0] * tex->lightmapVecsLuxelsPerWorldUnits[0][2];
    luxelSpaceCross[2] = tex->lightmapVecsLuxelsPerWorldUnits[1][0] * tex->lightmapVecsLuxelsPerWorldUnits[0][1] -
    ...
    else
    {
        ...
        l->luxelToWorldSpace[0][0] =
            ( l->facenormal[2] * l->worldToLuxelSpace[1][1] - l->facenormal[1] * l->worldToLuxelSpace[1][2] ) / det;
        l->luxelToWorldSpace[1][0] =
            ( l->facenormal[1] * l->worldToLuxelSpace[0][2] - l->facenormal[2] * l->worldToLuxelSpace[0][1] ) / det;
        l->luxelOrigin[0] = -( l->facedist * luxelSpaceCross[0] ) / det;
        l->luxelToWorldSpace[0][1] =
            ( l->facenormal[0] * l->worldToLuxelSpace[1][2] - l->facenormal[2] * l->worldToLuxelSpace[1][0] ) / det;
        l->luxelToWorldSpace[1][1] =
            ( l->facenormal[2] * l->worldToLuxelSpace[0][0] - l->facenormal[0] * l->worldToLuxelSpace[0][2] ) / det;
        l->luxelOrigin[1] = -( l->facedist * luxelSpaceCross[1] ) / det;
        l->luxelToWorldSpace[0][2] =
            ( l->facenormal[1] * l->worldToLuxelSpace[1][0] - l->facenormal[0] * l->worldToLuxelSpace[1][1] ) / det;
        l->luxelToWorldSpace[1][2] =
            ( l->facenormal[0] * l->worldToLuxelSpace[0][1] - l->facenormal[1] * l->worldToLuxelSpace[0][0] ) / det;
        l->luxelOrigin[2] = -( l->facedist * luxelSpaceCross[2] ) / det;

        // adjust for luxel offset
        VectorMA( l->luxelOrigin, -tex->lightmapVecsLuxelsPerWorldUnits[0][3], l->luxelToWorldSpace[0],
                  l->luxelOrigin );
        VectorMA( l->luxelOrigin, -tex->lightmapVecsLuxelsPerWorldUnits[1][3], l->luxelToWorldSpace[1],
                  l->luxelOrigin );
    }
...
void WriteCoordWinding( FILE *out, lightinfo_t *l, winding_t *w, Vector &color )
{
    ...
    for ( i = 0; i < w->numpoints; i++ )
    {
        ...
        fprintf( out, "%5.2f %5.2f %5.2f %5.3f %5.3f %5.3f\n", pos[0], pos[1], pos[2], color[0] / 256, color[1] / 256,
                 color[2] / 256 );
    }
}
...
void DumpFaces( lightinfo_t *pLightInfo, int ndxFace )
{
    ...
    for ( int ndxEdge = 0; ndxEdge < pLightInfo->face->numedges; ndxEdge++ )
    {
        ...
        CmdLib_FPrintf( out, "%f %f %f %f %f %f\n", p1[0], p1[1], p1[2], n1[0] * 0.5 + 0.5, n1[1] * 0.5 + 0.5,
                        n1[2] * 0.5 + 0.5 );

        CmdLib_FPrintf( out, "%f %f %f %f %f %f\n", p2[0], p2[1], p2[2], n2[0] * 0.5 + 0.5, n2[1] * 0.5 + 0.5,
                        n2[2] * 0.5 + 0.5 );

        CmdLib_FPrintf( out, "%f %f %f %f %f %f\n", centroid[0] + pLightInfo->modelorg[0],
                        centroid[1] + pLightInfo->modelorg[1], centroid[2] + pLightInfo->modelorg[2],
                        fn->facenormal[0] * 0.5 + 0.5, fn->facenormal[1] * 0.5 + 0.5, fn->facenormal[2] * 0.5 + 0.5 );
    }
...
bool BuildFacesamplesAndLuxels_DoFast( lightinfo_t *pLightInfo, facelight_t *pFaceLight )
{
    ...
    pFaceLight->worldAreaPerLuxel =
        1.0 /
        ( sqrt( DotProduct( pTex->lightmapVecsLuxelsPerWorldUnits[0], pTex->lightmapVecsLuxelsPerWorldUnits[0] ) ) *
          sqrt( DotProduct( pTex->lightmapVecsLuxelsPerWorldUnits[1], pTex->lightmapVecsLuxelsPerWorldUnits[1] ) ) );
...
bool BuildFacesamples( lightinfo_t *pLightInfo, facelight_t *pFaceLight )
{
    ...
    for ( int t = 0; t < height && pLightmapWinding; t++ )
    {
        ...
        for ( int s = 0; s < width && pWindingT2; s++ )
        {
            ...
            //
            // if winding T2 still exists free it and set it equal S1 (the rest of the row minus the sample just
            // created)
            //
            if ( pWindingT2 )
            {
        ...
        //
        // if the original lightmap winding exists free it and set it equal to T1 (the rest of the winding not cut into
        // samples)
        //
        if ( pLightmapWinding )
        {
...
entity_t *FindTargetEntity( char *target )
{
    ...
    for ( i = 0; i < num_entities; i++ )
    {
        ...
        if ( !strcmp( n, target ) ) return &entities[i];
    }
...
int LightForString( char *pLight, Vector &intensity )
{
    ...
    argCnt =
        sscanf( pLight, "%lf %lf %lf %lf %lf %lf %lf %lf", &r, &g, &b, &scaler, &r_hdr, &g_hdr, &b_hdr, &scaler_hdr );
    ...
    {  // point towards target
        ...
        if ( !e2 )
            Warning( "WARNING: light at (%i %i %i) has missing target\n", (int) dl->light.origin[0],
                     (int) dl->light.origin[1], (int) dl->light.origin[2] );
        else
        {
    ...
    if ( d50 )
    {
        ...
        if ( IntForKey( e, "_hardfalloff" ) )
        {
            ...
            dl->m_flStartFadeDistance =
                0.75 * d0 + 0.25 * d50;  // start fading 3/4 way between 50 and 0. could allow adjust
        }
        else
        {
...
bool CanLeafTraceToSky( int iLeaf )
{
    ...
    for ( int j = 0; j < NUMVERTEXNORMALS; j += 4 )
    {
        ...
        if ( TestSignSIMD( CmpGtSIMD( fractionVisible, Four_Zeros ) ) ) return true;
    }
...
void BuildVisForLightEnvironment( void )
{
    ...
    for ( int iLeaf = 0; iLeaf < numleafs; ++iLeaf )
    {
        ...
        if ( dleafs[iLeaf].flags & LEAF_FLAGS_SKY ) continue;

        // Don't bother with this leaf if it's solid
        if ( dleafs[iLeaf].contents & CONTENTS_SOLID ) continue;
        ...
        {
            if ( iLeaf2 == iLeaf ) continue;

            if ( !( dleafs[iLeaf2].flags & ( LEAF_FLAGS_SKY | LEAF_FLAGS_SKY2D ) ) ) continue;

            // Can this leaf see into the leaf with the sky in it?
            if ( !PVSCheck( pvs, dleafs[iLeaf2].cluster ) ) continue;
...
static char *ValueForKeyWithDefault( entity_t *ent, char *key, char *default_value = NULL )
{
    ...
    for ( ep = ent->epairs; ep; ep = ep->next )
        if ( !strcmp( ep->key, key ) ) return ep->value;
...
static void ParseLightEnvironment( entity_t *e, directlight_t *dl )
{
    ...
    if ( !gSkyLight )
    {
        ...
        if ( g_bHDR )
        {
            VectorScale( gAmbient->light.intensity, FloatForKeyWithDefault( e, "_AmbientScaleHDR", 1.0 ),
...
void CreateDirectLights( void )
{
    ...
    for ( i = 0; i < uiPatchCount; i++ )
    {
        ...
        // skip parent patches
        if ( p->child1 != g_Patches.InvalidIndex() ) continue;
        ...
        if ( VectorAvg( p->baselight ) >= dlight_threshold )
        {
            ...
            VectorScale( p->baselight, lightscale * p->area * p->scale[0] * p->scale[1] / p->basearea,
                         dl->light.intensity );
...
#define NSAMPLES_SUN_AREA_LIGHT \
    300  // number of samples to take for an
         // non-point sun light

// Helper function - gathers light from sun (emit_skylight)
void GatherSampleSkyLightSSE( SSE_sampleLightOutput_t &out, directlight_t *dl, int facenum, FourVectors const &pos,
                              FourVectors *pNormals, int normalCount, int iThread, int nLFlags,
...
void GatherSampleAmbientSkySSE( SSE_sampleLightOutput_t &out, directlight_t *dl, int facenum, FourVectors const &pos,
                                FourVectors *pNormals, int normalCount, int iThread, int nLFlags,
                                int static_prop_index_to_ignore, float flEpsilon )
{
    bool bIgnoreNormals = ( nLFlags & GATHERLFLAGS_IGNORE_NORMALS ) != 0;
    ...
    for ( int j = 0; j < nsky_samples; j++ )
    {
        ...
        if ( !TestSignSIMD( validity ) ) continue;
...
void GatherSampleStandardLightSSE( SSE_sampleLightOutput_t &out, directlight_t *dl, int facenum, FourVectors const &pos,
                                   FourVectors *pNormals, int normalCount, int iThread, int nLFlags,
                                   int static_prop_index_to_ignore, float flEpsilon )
{
    bool bIgnoreNormals = ( nLFlags & GATHERLFLAGS_IGNORE_NORMALS ) != 0;
    ...
    if ( !bIgnoreNormals ) dot = delta * pNormals[0];
    ...
    if ( bHasHardFalloff )
    {
        ...
        if ( !TestSignSIMD( notPastFadeDist ) ) return;
    }
    ...
    switch ( dl->light.type )
    {
        ...
        case emit_surface:
            ...
            if ( TestSignSIMD( CmpEqSIMD( Four_Zeros, dot ) ) == 0xF ) return;
...
void GatherSampleLightSSE( SSE_sampleLightOutput_t &out, directlight_t *dl, int facenum, FourVectors const &pos,
                           FourVectors *pNormals, int normalCount, int iThread, int nLFlags,
                           int static_prop_index_to_ignore, float flEpsilon )
{
    ...
    {
        case emit_skylight:
            GatherSampleSkyLightSSE( out, dl, facenum, pos, pNormals, normalCount, iThread, nLFlags,
                                     static_prop_index_to_ignore, flEpsilon );
            break;
        case emit_skyambient:
            GatherSampleAmbientSkySSE( out, dl, facenum, pos, pNormals, normalCount, iThread, nLFlags,
                                       static_prop_index_to_ignore, flEpsilon );
            break;
        case emit_point:
        case emit_surface:
        case emit_spotlight:
            GatherSampleStandardLightSSE( out, dl, facenum, pos, pNormals, normalCount, iThread, nLFlags,
                                          static_prop_index_to_ignore, flEpsilon );
            break;
        default:
...
void AddSampleToPatch( sample_t *s, LightingValue_t &light, int facenum )
{
    ...
    if ( VectorAvg( light.m_vecLighting ) < 1 ) return;

    //
    // fixed the sample position and normal -- need to find the equiv pos, etc to set up
    // patches
    //
    if ( g_FacePatches.Element( facenum ) == g_FacePatches.InvalidIndex() ) return;
    ...
    for ( patch = &g_Patches.Element( g_FacePatches.Element( facenum ) ); patch; patch = pNextPatch )
    {
        ...
        for ( i = 0; i < 3; i++ )
        {
            if ( mins[i] > s->pos[i] + radius ) goto nextpatch;
            if ( maxs[i] < s->pos[i] - radius ) goto nextpatch;
        }
...
void GetPhongNormal( int facenum, FourVectors const &spot, FourVectors &phongnormal )
{
    ...
    if ( smoothing_threshold != 1 )
    {
        ...
        for ( j = 0; j < f->numedges; ++j )
        {
            ...
            a1 = MulSIMD(
                a1, SubSIMD( MulSIMD( ReplicateX4( bb ), vspot * v1 ), MulSIMD( ReplicateX4( ab ), vspot * v2 ) ) );
            // a2 = (DotProduct( vspot, v2 ) - a1 * ab) / bb;
            ...
            if ( !TestSignSIMD( resultMask ) ) continue;
                ...
                for ( int iBump = 0; iBump < 4; ++iBump )
                {
                    if ( iBump == 0 || ( iBump > 0 && bBumpped ) )
                    {
                        for ( int iSample = 0; iSample < pFaceLight->numsamples; ++iSample )
                        {
                            ...
                            WriteWinding( pFileSamples[iStyle][iBump], pSample->w,
                                          pFaceLight->light[iStyle][iBump][iSample].m_vecLighting );
                            if ( bDumpNormals )
                            {
                                WriteNormal( pFileSamples[iStyle][iBump], pSample->pos, pSample->normal, 15.0f,
                                             pSample->normal * 255.0f );
                            }
                        }
                    }
                }
...
static inline int FindLightstyle( dface_t *f, int lightstyle )
{
    ...
    {
        if ( f->styles[k] == lightstyle ) return k;
    }
...
static int FindOrAllocateLightstyleSamples( dface_t *f, facelight_t *fl, int lightstyle, int numnormals )
{
    ...
    {
        if ( f->styles[k] == lightstyle ) break;
...
//-----------------------------------------------------------------------------
static void ComputeIlluminationPointAndNormalsSSE( lightinfo_t const &l, FourVectors const &pos,
                                                   FourVectors const &norm, SSE_SampleInfo_t *pInfo, int numSamples )
{
    Vector v[4];
    ...
    if ( computeNormals )
    {
        ...
        for ( int i = 0; i < 4; ++i )
        {
            ...
            GetBumpNormals( pInfo->m_pTexInfo->textureVecsTexelsPerWorldUnits[0],
                            ...
                            pInfo->m_PointNormals[0].Vec( i ), bv[i] );
        }
    ...
    for ( int i = 0; i < 4; ++i ) pInfo->m_Clusters[i] = ClusterFromPoint( pos.Vec( i ) );
}
...
//-----------------------------------------------------------------------------
static void ComputeIlluminationPointAndNormalsForDisp( lightinfo_t const &l, FourVectors &pos, FourVectors &norm,
                                                       SSE_SampleInfo_t *pInfo )
{
    ...
    if ( pInfo->m_NormalCount > 1 )
    {
        ...
        for ( int j = 0; j < 4; j++ )
        {
            // TODO: using Vec may slow things down a bit
            GetBumpNormals( pInfo->m_pTexInfo->textureVecsTexelsPerWorldUnits[0],
                            pInfo->m_pTexInfo->textureVecsTexelsPerWorldUnits[1], l.facenormal, norm.Vec( j ), bv[j] );
        }
        for ( int b = 0; b < NUM_BUMP_VECTS; b++ )
        {
            pInfo->m_PointNormals[b + 1].LoadAndSwizzle( bv[0][b], bv[1][b], bv[2][b], bv[3][b] );
    ...
    // TODO: this may slow things down a bit ( using Vec )
    for ( int j = 0; j < 4; j++ ) pInfo->m_Clusters[j] = ClusterFromPoint( pos.Vec( j ) );
}
...
static void GatherSampleLightAt4Points( SSE_SampleInfo_t &info, int sampleIdx, int numSamples )
{
    ...
    for ( directlight_t *dl = activelights; dl != NULL; dl = dl->next )
    {
        ...
        GatherSampleLightSSE( out, dl, info.m_FaceNum, info.m_Points, info.m_PointNormals, info.m_NormalCount,
                              info.m_iThread );
        ...
        int lightStyleIndex =
            FindOrAllocateLightstyleSamples( info.m_pFace, info.m_pFaceLight, dl->light.style, info.m_NormalCount );
        if ( lightStyleIndex < 0 )
        {
            if ( info.m_WarnFace != info.m_FaceNum )
            {
                Warning( "\nWARNING: Too many light styles on a face at (%f, %f, %f)\n", info.m_Points.x.m128_f32[0],
                         info.m_Points.y.m128_f32[0], info.m_Points.z.m128_f32[0] );
...
//-----------------------------------------------------------------------------
static void ResampleLightAt4Points( SSE_SampleInfo_t &info, int lightStyleIndex, int flags,
                                    LightingValue_t pLightmap[4][NUM_BUMP_VECTS + 1] )
{
    ...
    for ( directlight_t *dl = activelights; dl != NULL; dl = dl->next )
    {
        ...
        if ( dl->light.style != info.m_pFace->styles[lightStyleIndex] ) continue;
...
bool PointsInWinding( FourVectors const &point, winding_t *w, int &invalidBits )
{
    ...
    for ( int ndxPt = 1; ndxPt < w->numpoints; ndxPt++ )
    {
        ...
        if ( invalidBits == 0xF ) return false;
    }
...
//-----------------------------------------------------------------------------
static int SupersampleLightAtPoint( lightinfo_t &l, SSE_SampleInfo_t &info, int sampleIndex, int lightStyleIndex,
                                    LightingValue_t *pLight, int flags )
{
    ...
    for ( int i = 0; i < info.m_NormalCount; ++i ) pLight[i].Zero();
    ...
    {
        float aRow[4];
        for ( int coord = 0; coord < 4; ++coord ) aRow[coord] = csshift + coord * cscale;
        ...
        for ( int s = 0; s < 4; ++s )
        {
            ...
            int invalidBits = 0;
            if ( sample.w && !PointsInWinding( superSamplePosition, sample.w, invalidBits ) ) continue;
    ...
    else
    {
        ...
        int invalidBits = 0;
        if ( sample.w && !PointsInWinding( superSamplePosition, sample.w, invalidBits ) ) return 0;
...
//-----------------------------------------------------------------------------
static void ComputeLightmapGradients( SSE_SampleInfo_t &info, bool const *pHasProcessedSample, float *pIntensity,
                                      float *gradient )
{
    ...
    for ( int i = 0; i < fl->numsamples; i++ )
    {
        ...
        if ( pHasProcessedSample[i] ) continue;
...
//-----------------------------------------------------------------------------
static inline void ComputeLuxelIntensity( SSE_SampleInfo_t &info, int sampleIdx, LightingValue_t **ppLightSamples,
...
//-----------------------------------------------------------------------------
static void ComputeSampleIntensities( SSE_SampleInfo_t &info, LightingValue_t **ppLightSamples,
                                      float *pSampleIntensity )
{
...
static void BuildSupersampleFaceLights( lightinfo_t &l, SSE_SampleInfo_t &info, int lightstyleIndex )
{
    ...
    while ( do_anotherpass && pass <= extrapasses )
    {
        ...
        for ( int i = 0; i < info.m_pFaceLight->numsamples; ++i )
        {
            ...
            if ( pGradient[i] < 0.0625 ) continue;
            ...
            int ambientSupersampleCount =
                SupersampleLightAtPoint( l, info, i, lightstyleIndex, pAmbientLight, AMBIENT_ONLY );

            // Supersample the non-ambient light for each bump direction vector
            int directSupersampleCount =
                SupersampleLightAtPoint( l, info, i, lightstyleIndex, pDirectLight, NON_AMBIENT_ONLY );
    ...
    if ( l.isflat )
    {
        ...
        if ( info.m_NormalCount > 1 )
        {
            ...
            GetBumpNormals( info.m_pTexInfo->textureVecsTexelsPerWorldUnits[0],
                            info.m_pTexInfo->textureVecsTexelsPerWorldUnits[1], l.facenormal, l.facenormal,
...
void BuildFacelights( int iThread, int facenum )
{
    ...
    if ( !( g_FacesVisibleToLights[facenum >> 3] & ( 1 << ( facenum & 7 ) ) ) ) return;

    if ( texinfo[f->texinfo].flags & TEX_SPECIAL ) return;  // non-lit texture
    ...
    for ( int grp = 0; grp < numGroups; ++grp )
    {
        ...
        if ( !l.isflat )
        {
            for ( int i = 0; i < numSamples; i++ ) sample[i].normal = sampleInfo.m_PointNormals[0].Vec( i );
        }
    ...
    if ( g_pIncremental )
    {
        for ( dl = activelights; dl != NULL; dl = dl->next )
        {
            ...
            if ( dl->light.style == 0 ) g_pIncremental->FinishFace( dl->m_IncrementalID, facenum, iThread );
        }
    ...
    // improves continuity significantly between disp and brush surfaces, especially when using high frequency alpha
    // shadow materials
    if ( do_extra )
    {
...
void BuildPatchLights( int facenum )
{
    ...
    for ( k = 0; k < MAXLIGHTMAPS; k++ )
    {
        if ( f->styles[k] == 0 ) break;
    }

    if ( k >= MAXLIGHTMAPS ) return;
...
void PrecompLightmapOffsets()
{
    ...
    for ( facenum = 0; facenum < numfaces; facenum++ )
    {
        ...
        if ( texinfo[f->texinfo].flags & TEX_SPECIAL ) continue;  // non-lit texture

        if ( dlight_map != 0 ) f->styles[1] = 0;

        for ( lightstyles = 0; lightstyles < MAXLIGHTMAPS; lightstyles++ )
        {
            if ( f->styles[lightstyles] == 255 ) break;
        }
...
static void LinearToBumpedLightmap( const float *linearColor, const float *linearBumpColor1,
                                    const float *linearBumpColor2, const float *linearBumpColor3, unsigned char *ret,
                                    unsigned char *retBump1, unsigned char *retBump2, unsigned char *retBump3 )
{
...
void ConvertRGBExp32ToRGBA8888( const ColorRGBExp32 *pSrc, unsigned char *pDst, Vector *_optOutLinear )
{
    ...
    if ( _optOutLinear ) *_optOutLinear = linearColor;
}
